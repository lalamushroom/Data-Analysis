...
server <- function(input, output, session) {
  ...
  observe({
    ...
    plotDensity <- reactive({
      ...
      if (selected_variable == "subscribers") {
        fig1 <- ggplot(d1Test) + geom_density(aes(x = predsubscribers, color = as.factor(average_monthly_earnings_category)))
      } else if (selected_variable == "video.views") {
        fig1 <- ggplot(d1Test) + geom_density(aes(x = predvideo.views, color = as.factor(average_monthly_earnings_category)))
      } else if (selected_variable == "uploads") {
        fig1 <- ggplot(d1Test) + geom_density(aes(x = preduploads, color = as.factor(average_monthly_earnings_category)))
      } else if (selected_variable == "created_month") {
        fig1 <- ggplot(d1Test) + geom_density(aes(x = predcreated_month, color = as.factor(average_monthly_earnings_category)))
      } else {
        fig1 <- ggplot(d1Test) + geom_density(aes(x = predchannel_type, color = as.factor(average_monthly_earnings_category)))
      }
      return(fig1)
    })
